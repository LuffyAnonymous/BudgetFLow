 ...
 * uncategorized-merchant confidence penalty and land in Review Required.
 * Registering the recipient as a debt's payeeAlias exempts it from that
 * penalty (confidence-evaluator.ts) and, once posted, auto-applies the
 * payment to the matching debt (debt-linkage.ts) instead of requiring a
 * separate manual "Record Payment" click.
 */
describe("Debt payee auto-match", () => {
  ...
  it("does not touch any debt when the transfer recipient matches no registered payee", async () => {
    ...
    // penalty and lands in review, same as any other unrecognized expense.
